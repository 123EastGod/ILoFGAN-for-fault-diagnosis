...
def write_image(iterations, dir, im_ins, im_outs, format='jpeg'):
    ...
    image_grid = vutils.make_grid(image_tensor.data, nrow=K1+K2, padding=2, normalize=True)
...
'''
input:base_feature:[b,c,1*h*w]=(32*128*64)
dim = 2
index:feat_indices[b,num]
'''
def batched_index_select(input, dim, index):
    views = [input.shape[0]] + [1 if i != dim else -1 for i in range(1, len(input.shape))]  #[b,1,-1]
    expanse = list(input.shape)
    expanse[0] = -1
    expanse[dim] = -1   #[-1,c,-1]
    index = index.view(views)   #[b,1,num]
    index = index.expand(expanse)   #[b,c,num]
