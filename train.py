        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for it, (imgs, label) in enumerate(train_dataloader):
                ...
                if (iterations + 1) % config['snapshot_log_iter'] == 0:
                    ...
                    print("Iteration: [%06d/%06d], time: %d, loss_adv_dis: %03f, loss_adv_gen: %03f, loss_recon: %03f"
                          % (iterations + 1, max_iter, end-start, trainer.loss_adv_dis, trainer.loss_adv_gen, trainer.loss_recon))
                    write_loss(iterations, trainer, train_writer)       #待研究
