...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        cnn_f = [Conv2dBlock(3, nf, 5, 1, 2,     #改成相应灰度图
                             pad_type='reflect',
                             norm='sn',
                             activation='none')]
...
class Generator(nn.Module):
    ...
    def forward(self, xs):
        ...
        c, h, w = querys.size()[-3:]
        querys = querys.view(b, k, c, h, w)    #query[4,3,128,3,3]输入数据经过encoder得到的
        ...
        #feat_gen[4,128,4,4] fake_x[4,1,64,64]
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        model = [Conv2dBlock(3, 32, 5, 1, 2,    #改成相应灰度图[4*3,1,64,64]->[4*3,32,64,64]
                             norm='bn',
                             activation='lrelu',
                             pad_type='reflect'),
...
class LocalFusionModule(nn.Module):
    ...
    def forward(self, feat, refs, index, similarity):
        ...
        feat = feat.view(b, c, h, w)  # [b,c,h,w]=(32*128*8*8)实际[4,128,4,4]
