...
class Attention(nn.Module):
    ...
    def forward(self, x):
        x_real = x
        B, C, H, W = x.shape    #x[18,32,64,64]
        N = H*W
        #B, N, C     = x.shape
        x =x.permute(0,2,3,1).reshape((B,N,C))  #x[B,H*W,C]=[18,4096,32]
        x = self.norm(x)    #通道归一化，再注意力机制？
        qkv = self.qkv(x)   #qkv[B,3*H*W,C]
        qkv = qkv.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v     = qkv[0], qkv[1], qkv[2]    #q[B,h,H*W,C/h]

        attn = (q @ k.transpose(-2, -1)) * self.scale   #attn[B,h,H*W,H*W]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        '''
        所以多头的加入，将C个通道分成了h份，每一份独自相乘，得到独特的注意力权重，从而能够发现更多的相关特征，
        最后在拼接在一起，仍然保持独立性。
        所以在后来的特征融合，不同的通道也是进行独立的融合，意味着同样有h种不同切独立的融合方式，
        最后通过解码器将多通道解码成三通道图片。
        '''
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)     #(attn @ v)=[B,h,H*W,C/h]->x=[B,H*W,C]
        x = self.proj(x)
        x = self.proj_drop(x)
        x = x.permute(0, 2, 1).reshape((B, C, H, W)) + x_real
        return x
